...
const KeyInput = ({ value, onChange, placeholder = 'Enter encryption/decryption password' }) => (
  <div className="relative">
    ...
    <input type="password" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      className="w-full pl-10 pr-4 py-3 bg-slate-900 border border-slate-600/80 text-slate-200 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:outline-none text-sm" required />
  </div>
);
...
const TextFileCryptoPanel = ({ user }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    } finally { setProcessing(false); setKey(''); }
  };
  ...
  const handleSubmit = async (e) => {
   ...
   try {
      if (mode === 'encrypt') {
        ...
        await HistoryStore.addRecord(user.email, { file: originalName, action: `Decrypt ${payload.dimension === '3d' ? '3D Model' : 'Image'}`, mimeType });
