  ...
  const handleFile = (e) => {
    ...
    if (mode === 'encrypt') {
      if (dimension === '2d' && !FileProcessorCore.isImage(f))
        return setError('Please select a 2D image file (PNG, JPEG, GIF, etc.).');
      if (dimension === '3d' && !FileProcessorCore.is3D(f))
        return setError('Please select a 3D model file (.glb, .obj, .stl, .fbx).');
    } else {
        ...
        <input
          ...
          accept={
            mode === 'encrypt'
              ...
              : (dimension === '2d' ? '.png'    : '.glb,.gltf,.obj,.stl,.fbx,.encrypted.*,*')
          }
          onChange={handleFile}
