...
import { Lock, User, Mail, Eye, EyeOff, LogOut, Key, CheckCircle, FileText, Download, Image, Type, Shield, Database } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === 'encrypt') {
        ...
        FileProcessor.download({ text: compact, name: `${file.name}.aes256.txt` });
        ...
        FileProcessor.download({ text: plainText, name: FileProcessor.decryptedName(file.name) });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'encrypt') {
        ...
        const payload = await CryptoEngine.encrypt({ plainBytes, password: key });
        setOutputText(`${payload.salt}:${payload.iv}:${payload.ciphertext}`);
      } else {
        ...
        const payload = { salt, iv, ciphertext: rest.join(':'), algorithm: 'AES-256-GCM', iterations: 250000 };
        const plainBytes = await CryptoEngine.decrypt({ payload, password: key });
        setOutputText(new TextDecoder().decode(plainBytes));
      }
    } catch (err) {
      setError(err.message || 'Operation failed. Check your key and try again.');
    } finally {
        ...
        <div className="bg-slate-800 rounded-xl border border-slate-700/80 p-6 shadow-xl">
          <div className="mb-5">
            ...
            <p className="text-slate-400 text-xs mt-1">
              {activeTab === 'file' && 'Encrypt or decrypt .txt files using AES-256-GCM. Supports all characters including colons, semicolons, and special symbols.'}
              ...
              {activeTab === 'dataset' && 'Encrypt or decrypt dataset files (.csv, .xlsx) using AES-256-GCM. Preview your data before encrypting.'}
            </p>
          </div>
          {activeTab === 'file' && <TextFileCryptoPanel />}
          {activeTab === 'textarea' && <TextAreaCryptoPanel />}
          {/* {activeTab === 'image' && <ImageCryptoPanel />} */}
          {activeTab === 'dataset' && <DatasetCryptoPanel />}
        </div>
