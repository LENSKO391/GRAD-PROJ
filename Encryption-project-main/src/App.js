      ...
      <div>
        ...
        <div className="relative">
          ...
          <input type={showKey ? 'text' : 'password'} value={key} onChange={e => setKey(e.target.value)}
            placeholder="Enter encryption password or generate one"
...
const ImageCryptoPanel = ({ user, forcedDimension = null }) => {
  const [mode, setMode] = useState('encrypt');
  const [dimension, setDimension] = useState(forcedDimension || '2d');
  const [file, setFile] = useState(null);
  const [fileInputKey, setFileInputKey] = useState(0);
  const [key, setKey] = useState('');
  const [showKey, setShowKey] = useState(false);
...
const TAB_DESCRIPTIONS = {
  ...
  image: 'Encrypt or decrypt 2D image files (PNG, JPEG, GIF, etc.) with visual distortion.',
  model: 'Encrypt or decrypt 3D model files (.glb, .obj, .stl, .fbx) with geometric distortion.',
